//golangcitest:args -Eerrgroupctx
package testdata
...
import (
	...
	erGr "golang.org/x/sync/errgroup"
)
...
func MultipleCtxArgs() {
	...
	_ = egCtx
	eg.Go(func() error {
		return doSmth2(ctx1, ctx2) // want `errgroup callback should probably not reference outer context "ctx1", use the errgroup-derived context "egCtx"`
	})
	eg.Wait()
}
